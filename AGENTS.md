...
7. **Agent-agnostic.** Adding/removing an agent type means editing exactly
   ...
   forge CLI (`gh`, `glab`, `tea`, …) goes through the `forge_*` dispatch
   family — `forge_provider`, `forge_bin`, `forge_label`, `forge_create_pr`,
   `forge_view_pr`, `forge_pr_checks_watch`, `forge_merge_pr`,
   `forge_pr_ci_status`. Adding a new forge = filling in its case in each
   helper — never inline `gh`/`glab`/`tea` outside this family. Auto-
   detection reads `git remote get-url origin`; override with `$YAAMUX_FORGE`.
   Currently supported: github (full), gitlab (create+merge), gitea (create).
   Tracked in #25 — Phase 1 = forge; Phase 2 = notifier; Phase 3 = extensions.
